	...
	typedef struct _stPbDvrLoginInfo
	{
		...
		char szRev[160];                    //预留
 		_stPbDvrLoginInfo()
 		{
			...
			memset(szAccessKey, 0, sizeof(szAccessKey));
			memset(szSecretKey, 0, sizeof(szSecretKey));
			memset(szBucket, 0, sizeof(szBucket));
			memset(szObjectKey, 0, sizeof(szObjectKey));
