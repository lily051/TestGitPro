...
void operator ()(const char* message, _HLW_ARG2_N(const V, & v)) \
{ \
    ...
    do \
    { \
        message = ParsePattern(message, placeholderNo, placeholderPattern, outBuffer); \
        if (placeholderNo >= 0) \
        { \
...
void operator ()(const T& policy, const char* message, _HLW_ARG2_N(const V, & v)) \
{ \
    if ((NULL == message) || (false == _logger.IsEnabledFor(_level))) \
    { \
        return; \
...
public:
    PolicyLocationLogger(HlogWrapper& logger, HLOG_LEVEL level,
        const char* filename, const char* functionName, int lineNumber)
        : LocationLogger(logger, level, filename, functionName, lineNumber)
    {
    }
