...
#if defined OS_WINPHONE
		::InterlockedIncrementAcquire(const_cast<long *>(&(this->value_)));
#else
...
#if (defined _WIN32 || defined _WIN64)
#if defined OS_WINPHONE
		long value = ::InterlockedDecrementAcquire(const_cast<long *>(&(this->value_)));
#else
...
#if (defined _WIN32 || defined _WIN64)
#if defined OS_WINPHONE
		return ::InterlockedExchangeAddAcquire(const_cast<long *>(&(this->value_)), 0);
#else
...
//
template<typename T_Class, typename endclass = shared_object, typename T_Lock = hpr::hpr_mutex>
class smartptr
{
public:
