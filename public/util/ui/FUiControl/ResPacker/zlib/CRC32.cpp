...
unsigned long ISO_13818_CRC32::get_value() const  
{   
    return   crc_reg;   
}   
...
unsigned char ISO_13818_CRC32::pump_bit(unsigned   char   b,   unsigned int   bit_pos) const  
{//返回b的第8-bit_pos位（从高位到低位）   
    if (bit_pos <= 7)   
    {   
        if (b & ((unsigned int)1 << bit_pos))   
        {   
            return 1;   
        }   
        else   
...
void ISO_13818_CRC32::update(const unsigned char* data, long length)   
{   
    crc_reg = 0xFFFFFFFF;   
