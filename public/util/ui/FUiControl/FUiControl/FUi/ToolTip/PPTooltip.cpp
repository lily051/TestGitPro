...
//	EnableTextWrap(FALSE);
	...
	SetDelayTime(PPTOOLTIP_TIME_AUTOPOP, 8000);
...
void CPPToolTip::OnRedrawTooltip(HDC hDC, BYTE nTransparency /* = 0 */)
{
	...
	//RUS: Если значение прозрачности больше максимальн?допустимог?
	...
	//RUS: Получаем размер тултип?бе?тени
...
//	GetWindowRgn(hrgn);
	...
	if (NULL == m_hUnderTooltipBk)
	{
		...
		//RUS: Сохраняем фо?по?тултипом ?битмап
	...
	else
	{
		...
		//RUS: Восстанавливае?фо?из битмап?
		::SelectObject(hBkDC, m_hUnderTooltipBk);
	} //if

	//ENG: Copy a original background bitmap to the temporary DC
	//RUS: Копируем оригинальный битмап во временны?контекст па?ти
	...
	//RUS: Накладывае?тултип на клиентскую част??альф?наложением
...
BOOL CPPToolTip::RelayEvent(MSG* pMsg)
{
	...
	//RUS: Запрет обработк?сообщени?если объект не создан
	if (NULL == GetSafeHwnd())  
		return FALSE;
    ...
    case WM_NCMOUSEMOVE:
		...
		if ((m_ptOriginal.x != pMsg->pt.x) || (m_ptOriginal.y != pMsg->pt.y))
		{
			...
			else if (IsCursorOverTooltip() && !(m_tiDisplayed.nBehaviour & PPTOOLTIP_TRACKING_MOUSE)) 
			{
				...
				//RUS: Курсор на?тултипом пр?выключенно?режиме "тракинга"
				if (!(m_tiDisplayed.nBehaviour & PPTOOLTIP_NOCLOSE_OVER))
				{
					//ENG: A tooltip don't hides when mouse over him
					//RUS: Если не установлен стил?не закрыван? тултип?если курсор на?ни?
					HideTooltip();
				}
				else
				{
					//ENG: Resetup autopop timer
					//RUS: Переустанавливае?таймер автозакрыт? тултип?
					SetAutoPopTimer();
				} //if
			}
			else
			{
				...
				//RUS: Ищем элемен?на панели инструментов
				hWnd = FindToolBarItem(pMsg->pt, ti);
				if (NULL == hWnd)
				{
					//ENG: Searching a hot area of the tooltip
					//RUS: Ищем предопределенную го?чу?зону тултип?
					hWnd = FindTool(&pt, ti);
				} //if
				//TRACE ("Временно?окно = 0x%08X\n", hWnd);
				if (NULL == hWnd)
				{
					//ENG: An item with a tooltip wasn't found
					//RUS: Ни один элемен? отображающий тултип, не найден
					m_hwndDisplayedTool = NULL;
				...
				else 
				{
					if ((hWnd != m_hwndDisplayedTool) || (ti.rectBounds != m_tiDisplayed.rectBounds/* m_rcDisplayedTool*/))
					{
						...
						//RUS: Если ново?окно ил?новы?элемен?окна, то установить новы?тултип
						SetNewTooltip(hWnd, ti);
					}
					else
					{
						//ENG: Nothing was changed
						//RUS: Если ни окно, ни элемен?окна не изме?лись
						if (m_tiDisplayed.nBehaviour & PPTOOLTIP_TRACKING_MOUSE)
						{
							//ENG: If sets tracking mode
							//RUS: Если установлен режи?"тракинга"
							SetAutoPopTimer();
						...
						else if (!(m_tiDisplayed.nBehaviour & PPTOOLTIP_CLOSE_LEAVEWND))
						{
							...
							//RUS: Если должен прятать? прилюбом движении мыши
							if ((hWnd == m_hwndDisplayedTool) && 
								!(m_tiDisplayed.nBehaviour & PPTOOLTIP_MULTIPLE_SHOW))
							{
								//ENG: "Multiple show" mode was disabled
								//RUS: Если не установлен режи?множественного показа тултип?
								HideTooltip();
							}
							else
							{
								//ENG: "Multiple show" mode was enabled
								//RUS: Если установлен режи?множественного показа тултип?
								SetNewTooltip(hWnd, ti);
							} //if
						}
						else
						{
							...
							//RUS: Тултип не должен прятать? пока находится на?окно?
							SetAutoPopTimer();
						} //if
					} //if
				} //if
...
void CPPToolTip::SetNewTooltip(HWND hWnd, const PPTOOLTIP_INFO & ti, BOOL bDisplayWithDelay /* = TRUE */, TooltipType type /* = PPTOOLTIP_NORMAL */)
{
	...
	//RUS: Начинаем пока?нового тултип?
	if (bDisplayWithDelay && m_dwTimeInitial)
...
void CPPToolTip::OnTimer(UINT nIDEvent) 
{
	...
	case TIMER_SHOW:
		...
		//ENG: Kill SHOW timer 
		//RUS: Убит?таймер ожидан? показа тултип?
		KillTimer(TIMER_SHOW);
		//ENG: Get current mouse coordinates
		//RUS: Получить текуще?положени?тултип?
	...
	case TIMER_SHOWING:
		...
		//RUS: Если шаги эффект?плавного показа исчерпан? то установить минимальну?прозрачность
		...
		if (m_dwCurTransparency <= m_tiDisplayed.nTransparency)
		{
			...
			//RUS: Убивае?таймер плавного показа ?устанавливае?сост?ни?тултип?ка?SHOWN
			...
			//RUS: Запуск таймер?на автоматическое сокрытие тултип?
			SetAutoPopTimer();
		} //if
		//ENG: Redraw tooltip with new transparency factor
		//RUS: Перерисовать текущи?тултип ?новы?коэффициенто?прозрачности
		OnRedrawTooltip(NULL, m_dwCurTransparency);
		break;	
	case TIMER_HIDE:
		TRACE(_T("OnTimerHide()\n"));
		//ENG: Kill all timers except HIDING timer
		//RUS: Убивае?вс?таймер?за исключение?таймер?HIDING
		KillTimer(TIMER_SHOW);
		KillTimer(TIMER_SHOWING);
		KillTimer(TIMER_HIDE);
		//ENG: If hiding timer don't start
		//RUS: Проверяем запуще?ли таймер сокрыт? тултип?
		if (PPTOOLTIP_STATE_HIDING != m_nTooltipState)
		{
			...
			if ((SHOWEFFECT_FADEINOUT == m_dwHideEffect) && m_dwTimeFadeOut)
			{
				...
				//RUS: Если эффект плавного сокрыт? разрешен ?указан?время шага сокрыт?
				SetTimer(TIMER_HIDING, m_dwTimeFadeOut, NULL);
			}
			else
			{
				//ENG: Sets a maximum transparency and to stops a hiding of a tooltip
				//RUS: Устанавливае?максимальную прозрачность ?останавливае?сокрытие
	...
	case TIMER_HIDING:
		...
		//RUS: Если шаги эффект?плавного сокрыт? исчерпан? то установить максимальную прозрачность
		...
		if (PERCENT_MAX_TRANSPARENCY == m_dwCurTransparency)
		{
			...
			//RUS: Убивае?таймер плавного сокрыт? ?пряче?окно тултип?
			...
			if (m_bNextToolExist)
			{
				...
				//RUS: Если существует подготовленный тултип, то начать ег?отображени?
				m_bNextToolExist = FALSE;
//				m_nTooltipType = m_nNextTooltipType;
//				m_nNextTooltipType = PPTOOLTIP_NORMAL;
...
void CPPToolTip::HideTooltip()
{
	...
	case PPTOOLTIP_STATE_SHOWING:
		...
		//RUS: Если тултип только отбражается, то прекращаем ег?отображать
		KillTimer(TIMER_SHOWING);
...
void CPPToolTip::PrepareDisplayTooltip(LPPOINT lpPoint)
{
	...
	//RUS: Вычисление положения тултип?на экране
	...
	//RUS: Если плавно?отображени?доступно
	if ((SHOWEFFECT_FADEINOUT == m_dwShowEffect) && m_dwTimeFadeIn)
	...
	if (m_szOffsetShadow.cx || m_szOffsetShadow.cy)
	{
		...
		//RUS: Увеличивае?размер?тултип?для рисования тени
	...
	//RUS: Устанавливае?тултип на экране
...
DWORD CPPToolTip::GetTooltipDirection(DWORD dwDirection, const CPoint & ptPoint, CPoint & ptAnchor, CRect & rcBody, CRect & rcFull, CRect & rcTipArea)
{
	...
	//RUS: Инициализация полног?размер?ограничивающег?тултип прямоугольник?
	rcFull = rcBody;
	switch(dwDirection) 
	{
	case PPTOOLTIP_LEFTEDGE_TOP:
	...
	//RUS: Проверка горизонтальных размеров на попадани??экра?
	CPoint pt(ptPoint);
	switch(dwDirection) 
	{
	case PPTOOLTIP_LEFTEDGE_TOP:
	...
	//RUS: Проверка вертикальных размеров на попадани??экра?
	switch(dwDirection) 
	{
	case PPTOOLTIP_LEFTEDGE_TOP:
	case PPTOOLTIP_RIGHTEDGE_TOP:
	...
	//RUS: Установк?координаты кончик?
	switch(dwDirection) 
	{
	case PPTOOLTIP_LEFTEDGE_TOP:
	case PPTOOLTIP_LEFTEDGE_VCENTER:
...
HWND CPPToolTip::GetWndFromPoint(const LPPOINT lpPoint, PPTOOLTIP_INFO & ti, BOOL bCheckTool /* = TRUE */)
{
	...
	if (NULL != hWnd)
	{
		...
		// lpPoint->y >= 0判断NCButton的ToolTip
		if (NULL != hWndTemp && lpPoint->y >= 0)
		{
            // 不是下拉控件
        ...
        {
            ::ScreenToClient(hWnd, &pt);
            if (FindTool(hWnd, &pt, ti) || !bCheckTool)
                return hWnd;
        }
	} //if
...
BOOL CPPToolTip::FindTool(HWND hWnd, const LPPOINT lpPoint, PPTOOLTIP_INFO & ti)
{
	...
	if ((hotarea.size() == 1) && hotarea[0].rectBounds.IsRectEmpty())
	{
		...
		//RUS: Если ограничивающий прямоугольник го?че?зоны не определе?
		ti = hotarea[0];
		return TRUE;
	} //if
	...
	if (hWnd != m_hParentWnd)
	{
		...
		//RUS: Если HWND не относится ?родительском?окну, то преобразуе?координаты
	...
	if (pScroll->IsKindOf(RUNTIME_CLASS(CScrollView))) 
	{
		...
		//RUS: Если HWND принадлежи?CScrollView ил?производному от него классу, то корректируем координаты
	...
	//RUS: Ищем го?чу?зону по?курсором
			...
			if (m_wndToolBars[i] == hWnd)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					if (rect.PtInRect(point))
					{
						...
						//RUS: Элемен?панели инструментов обнаруже?
...
void CPPToolTip::AddTool(CWnd * pWnd, PPTOOLTIP_INFO & ti)
{
	...
	if (item == m_ToolMap.end())
	{
		...
		//RUS: Тултип для указанного HWND не обнаруже? поэтом?создае?ег?
		arHotArea hotarea;
	...
	//RUS: Получаем параметр?тултип?
	arHotArea & hotarea = item->second;

	//ENG: A tooltip has more one rectangle areas. Check all theirs
	//RUS: Тултип содержит боле?одно?прямоугольной област? проверим всех их
	arHotArea::iterator iter;
	for (iter = hotarea.begin(); iter != hotarea.end(); ++iter)
	{
		if (/*ti.rectBounds == (*iter).rectBounds && */ti.nIDTool == (*iter).nIDTool)
		{
			//ENG: Specified window's rect already exist and so updates him
			//RUS: Указанны?прямоугольник окна уж?существует, поэтом?просто обновляем ег?параметр?
			*iter = ti;
			return;
		} //if
	} //for
...
void CPPToolTip::RemoveTool(CWnd * pWnd, LPCRECT lpRectBounds /* = NULL */)
{
	...
	if (NULL == lpRectBounds)
	{
		...
		//RUS: Удаляют? вс?инструмент?для указанного окна
		m_ToolMap.erase(item);
	}
	else
	{
		//ENG: Search the tool to remove
		//RUS: Поис?инструмент?для удален?
		arHotArea & hotarea = item->second;
		arHotArea::iterator iter;
		for (iter = hotarea.begin(); iter != hotarea.end(); ++iter)
		{
			if ((*iter).rectBounds == *lpRectBounds)
			{
				...
				if (hotarea.size() > 1)
				{
					...
					//RUS: Если указанно?окно содержит боле?одно?област? то удаляем только указанну?област?
					hotarea.erase(iter);
				}
				else
				{
					//ENG: If the specified window has one rectangle area only then removes the tool for specified window
					//RUS: Если указанно?окно имее?только одну прямоугольную област? то удалит?весь инструмент
					m_ToolMap.erase(item);
				} //if
				return;
			} //if
...
void CPPToolTip::RemoveAllTools()
{
	...
	//RUS: Удаляем вс?инструмент?
	if (m_ToolMap.size())
		m_ToolMap.clear();

	//ENG: Removes all toolbars
	//RUS: Удаляем вс?панели инструментов
	if (m_wndToolBars.size())
		m_wndToolBars.clear();
...
void CPPToolTip::AddToolBar(CToolBar * pBar)
{
	...
	//RUS: Поис?дубликат?указанно?панели инструментов
	...
	//RUS: Запрещае?стандартны?подсказк?для указаной панели инструментов
...
void CPPToolTip::SetDefaultSizes(BOOL bBalloonSize /* = TRUE */)
{
	...
	else
	{
		...
		SetSize(PPTTSZ_MARGIN_CX, 5);
		SetSize(PPTTSZ_MARGIN_CY, 5);
