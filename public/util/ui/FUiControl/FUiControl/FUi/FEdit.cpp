...
IMPLEMENT_DYNAMIC(CFEdit, CEdit)
...
BEGIN_MESSAGE_MAP(CFEdit, CEdit)
    ...
    ON_WM_SETFOCUS()
    ON_WM_KILLFOCUS()
    ON_CONTROL_REFLECT(EN_CHANGE, &CFEdit::OnEnChange)
...
// CFEdit 消息处理程序
void CFEdit::OnNcCalcSize(BOOL /*bCalcValidRects*/, NCCALCSIZE_PARAMS* lpncsp)
{
    VCenterEdit(m_bMultiLine, lpncsp);
}

/*在该处处理上下两条颜色刷新，未完成*/
void CFEdit::OnNcPaint()
{
    // 获取整个控件的DC，包括非客户区
    CDC *pDC = GetWindowDC();
    if (NULL == pDC)
...
// 获得焦点
void CFEdit::OnSetFocus(CWnd* pOldWnd)
{
...
// 失去焦点
void CFEdit::OnKillFocus(CWnd* pNewWnd)
{
...
HBRUSH CFEdit::CtlColor(CDC* pDC, UINT /*nCtlColor*/)
{
...
// 设置显示字体
void CFEdit::SetTextFont(int nState, const CString& strFontName)
{
...
// 设置显示文字颜色
void CFEdit::SetTextColor(int nState, COLORREF color)
{
    if (nState & FUi::Normal)
    {
...
// 设置提示文字
void CFEdit::SetDefaultText(LPCTSTR szTipText)
{
...
// 设置边框颜色
void CFEdit::SetBorderColor(int nState, COLORREF color)
{
    if (nState & FUi::Normal)
    {
...
// 设置是否错误提示状态
void CFEdit::SetErrorState(BOOL bError /*= FALSE*/)
{
    m_bError = bError;
    SetFocus();
}
// 画边框
void CFEdit::DrawBorder(CDC* pDC, CRect& rcWnd)
{
    if (NULL == pDC)
    {
        return ;
...
//lint -e569
void CFEdit::SetPasswordMode(BOOL bSet)
{
    if (bSet)
	{
		m_bPassword = TRUE;
...
void CFEdit::GetPasswordText(CString& strText)
{
    m_bCanGetText = TRUE;
...
void CFEdit::VCenterEdit(BOOL bFlag, NCCALCSIZE_PARAMS* lpncsp)
{
...
void CFEdit::OnSize(UINT nType, int cx, int cy)
{
...
LRESULT CFEdit::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case EM_SETSEL:
...
void CFEdit::SetTipText(LPCTSTR szTipText)
{
