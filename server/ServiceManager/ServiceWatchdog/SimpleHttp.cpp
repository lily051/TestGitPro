...
bool CSimpleHttp::Connect(const std::string &strHost, HPR_UINT16 nPort/* = 80*/)
{
    ...
    do 
    {
        ...
        // 连接时长调整到2s，失败了走报警上传中心逻辑
        if (HPR_ConnectWithTimeOut(m_client, &addr, 3000) != 0)
        {
            stringstream ss;
...
bool CSimpleHttp::AddCenterAlarmLog(const center_info_t &centerInfo, alarm_msg_t &stAlarm)
{
    //// strcomment信息转换为URI
    //const int nLength = 5*1024;
    //char szComments[nLength];
    //sprintf_s(szComments, "%s", fcT2A(stAlarm.strInfo.c_str()));
    //const char* pTemp = encodeURI(szComments);// 一次转多字节
    //_snprintf_s(szComments, sizeof(szComments), _TRUNCATE, "%s", pTemp);
    //const char* pUser = encodeURI(szComments);// 二次转码URI
    //_snprintf_s(szComments, sizeof(szComments), _TRUNCATE, "%s", pUser);

    std::string strResult;
    std::string strWebAddr ="/dataCenter!saveAlarmInfo";
    char szContents[6*1024]={0};
    sprintf_s(szContents, 
        "ip=%s&port=%d&alarmlog.id=%I64d&alarmlog.checkType=%d&alarmlog.checkChan=%d&alarmlog.contactObjId=%d&alarmlog.alarmType=%d&alarmlog.alarmTime=%s&alarmlog.alarmRegionId=%d&alarmlog.comments=%s&alarmlog.status=%d&alarmlog.strGuid=%s",
        fcT2U(stAlarm.strIP.c_str()),
        stAlarm.nport,
        stAlarm.nLastAlarmID,
        stAlarm.nParentKey,
        stAlarm.nChannel,
        stAlarm.nRev,
        stAlarm.nKey,
    ...
    VIEWMSGLOG_INFO("AddRecordInfo:%s", fcU2A(strContents.c_str()));
    if(!CSimpleHttp::PostMsg(fcT2A(centerInfo.ip.c_str()),
        (HPR_UINT16)(centerInfo.port),
        strWebAddr,
        strContents,
        strResult))
    {
    ...
    {
        VIEWMSGLOG_INFO("AddRecordInfo:%s", fcU2A(strResult.c_str()));
        CMarkup xml;
        xml.SetDoc(fcA2T(strResult.c_str()));
        xml.ResetMainPos();

        xml.FindChildElem(fcA2T("success"));
        std::_tstring strsuccess = (LPCTSTR)xml.GetChildData();
        if (strcmp(fcT2A(strsuccess.c_str()), "true") != 0)
        {
            VIEWMSGLOG_ERROR("AddRecordInfo failed");
            return false;
        }

        xml.FindChildElem(fcA2T("centerId"));
        stAlarm.nCurrAlarmID = _atoi64(fcT2A((LPCTSTR)xml.GetChildData()));
