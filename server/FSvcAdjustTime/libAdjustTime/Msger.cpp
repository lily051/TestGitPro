					...
					LPCTSTR pszFormat, ...)
{
	...
	//模块
	sprintf_s(szBuff, sizeof(szBuff), "%d", SVC_ADJUST);
	...
	//报警ip地址
	msgAlarm.set_alarmhost(fcA2U(GetConfigPtr()->m_strLocalIP.c_str()));

	GetClientSessionMgrPtr()->SendAlarmMsg(GetConfigPtr()->m_strATServerIP, 
		GetConfigPtr()->m_nATServerPort, AlarmMsg::AUTHORIZE_REQ, &msgAlarm);
}

void Msger_DeviceAlarmMsg(const DWORD alarmType,
                    const DWORD userid, 
                    const DWORD channel, 
                    const DWORD alarmStatus,
                    const DWORD dwRev,
                    const char* pDeviceIp,
                    const DWORD dwIpSize,
                    LPCTSTR pszFormat, ...)
{
	...
	//报警ip地址
	msgAlarm.set_alarmhost(pDeviceIp, dwIpSize);
