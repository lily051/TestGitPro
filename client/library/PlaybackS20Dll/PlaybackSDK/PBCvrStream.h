...
class CPBCvrStream : public CPlayStream
{
public:
	CPBCvrStream(void);
	~CPBCvrStream(void);
...
protected:
	...
	virtual BOOL StartData(LONG lLogin, const char* pStreamID, NET_DVR_TIME& stNetStartTime, NET_DVR_TIME& stNetStopTime
		, BOOL bConvertPlay = FALSE, BYTE byResolution = 0, DWORD dwVideoBitrate = 0, DWORD dwVideoFrameRate = 0);
	// 停止取流.
...
public:
    ...
    // 按时间查找文件.
    BOOL FindFileByTime(FINDFILE_PARAM& stFindFileParam, LPB_FILEINFO& lpFileList, int& nFileCount);
	// 按事件查找文件.
...
extern void CALLBACK pCvrStream(LONG lPlayHandle, DWORD dwDataType, BYTE * pBuffer, DWORD dwBufSize, DWORD dwUser);