...
typedef struct _PBSDDvrLoginInfo
{
	...
	char szAccessKey[256];             //AccessKey(对象云存储专用)
	char szSecretKey[256];             //SecretKey(对象云存储专用)
	char szBucket[256];                //Bucket(对象云存储专用)
	char szObjectKey[256];             //ObjectKey(对象云存储专用，唯一标识)
	char szRev[160];                    //预留
    _PBSDDvrLoginInfo()
    {
		...
		ZeroMemory(szAccessKey, sizeof(szAccessKey));
		ZeroMemory(szSecretKey, sizeof(szSecretKey));
		ZeroMemory(szBucket, sizeof(szBucket));
		ZeroMemory(szObjectKey, sizeof(szObjectKey));
...
typedef struct _PBSDTimeParam
{
    ...
    _PBSDTimeParam()
    {
		...
		dwRev = 0;
        emFileType = PBSD_RECORD_TYPE_ALL;
    }
