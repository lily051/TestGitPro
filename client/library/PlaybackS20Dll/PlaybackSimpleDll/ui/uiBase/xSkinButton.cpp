...
void CxSkinButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
    ...
    {	// DISABLED BUTTON
        ...
        if (m_Checked) OffsetRect(&tr,1,1);
        ...
        CRect rc;
        GetClientRect(&rc);		
        DrawIconEx(pDC->m_hDC, rc.left + 6, rc.top + 6, m_hDisableIcon, 16, 16, 0, NULL, DI_NORMAL | DI_COMPAT);
    ...
    {										// SELECTED (DOWN) BUTTON
        ...
        CRect rc;
        GetClientRect(&rc);				
        DrawIconEx(pDC->m_hDC, rc.left + 6, rc.top + 6, m_hNormalIcon, 16, 16, 0, NULL, DI_NORMAL | DI_COMPAT);

        if (m_pImage)
        {
            Graphics graph(pDC->m_hDC);
            int x = (rc.Height() - 32) / 2;
            graph.DrawImage(m_pImage, rc.left+5,x+8,16,16);
            tr.left += 16;
        }
                           ...
                           short drawmode, short border, short margin)
{
    ...
    {
        SetWindowPos(NULL,0,0,GetBitmapWidth(m_bNormal),
            GetBitmapHeight(m_bNormal),SWP_NOZORDER|SWP_NOMOVE);
    }

}

void CxSkinButton::SetNormal(HBITMAP hNormal)
{
...
void CxSkinButton::SetSkinBitmap(HBITMAP hNormal, HBITMAP hDown, HBITMAP hHover, HBITMAP hDisable)
{
    m_bNormal.DeleteObject();
    m_bDown.DeleteObject();
    m_bOver.DeleteObject();
    ...
    m_bOver.Attach(hHover);
	if (NULL != hDisable)
	{
		m_bDisabled.Attach(hDisable);
	}
...
void CxSkinButton::SetIcon(UINT uNormal)
{
    m_hNormalIcon = AfxGetApp()->LoadIcon(uNormal);

    m_hDisableIcon = CreateGrayscaleIcon(m_hNormalIcon);
}

HICON CxSkinButton::CreateGrayscaleIcon(HICON hIcon)
{
