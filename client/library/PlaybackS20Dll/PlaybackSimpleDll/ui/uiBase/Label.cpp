...
CLabel& CLabel::SetFontSize(int nSize)
{
	...
	lf.lfHeight = nSize*2;                      // request a 12-pixel-height font
...
{	
    ...
    if (strFont.GetLength() <= LF_FACESIZE)
    {
...
void CLabel::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
//ljg 2010/05/05 add [[
//设置logo的内容，并设置为可显示的 
void CLabel::SetLogo(CString strLogo)
{
	m_sLogo = strLogo;
	m_bShowLogo = TRUE;
}
//是否显示logo
void CLabel::ShowLogo(BOOL bShow /*= TRUE*/)
{
	m_bShowLogo = bShow;
}
//]]end