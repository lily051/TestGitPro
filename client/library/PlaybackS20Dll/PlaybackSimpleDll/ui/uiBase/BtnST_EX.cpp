...
#endif
IMPLEMENT_DYNCREATE(CButtonST_EX,CButton)
CButtonST_EX::CButtonST_EX()
{
	m_bIsPressed		= FALSE;
	m_bIsFocused		= FALSE;
	m_bIsDisabled		= FALSE;
...
BEGIN_MESSAGE_MAP(CButtonST_EX, CButton)
    //{{AFX_MSG_MAP(CButtonST_EX)
	ON_WM_SETCURSOR()
	ON_WM_KILLFOCUS()
	ON_WM_MOUSEMOVE()
...
void CButtonST_EX::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		// Destroy icons
...
void CButtonST_EX::PreSubclassWindow() 
{
	UINT nBS;
...
UINT CButtonST_EX::OnGetDlgCode() 
{
...
BOOL CButtonST_EX::PreTranslateMessage(MSG* pMsg) 
{
	// ==============2009-08-15 删除该行为================
// 	InitToolTip();
...
HBRUSH CButtonST_EX::CtlColor(CDC* pDC, UINT nCtlColor) 
{
	return (HBRUSH)::GetStockObject(NULL_BRUSH); 
} // End of CtlColor

void CButtonST_EX::OnSysColorChange() 
{
...
LRESULT CButtonST_EX::OnSetStyle(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CButtonST_EX::OnSetCheck(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CButtonST_EX::OnGetCheck(WPARAM wParam, LPARAM lParam)
{
	ASSERT(m_bIsCheckBox);
	return GetCheck();
} // End of OnGetCheck
...
#ifdef	BTNST_USE_BCMENU
LRESULT CButtonST_EX::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu) 
{
	LRESULT lResult;
...
#ifdef	BTNST_USE_BCMENU
void CButtonST_EX::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
	BOOL bSetFlag = FALSE;
...
void CButtonST_EX::OnEnable(BOOL bEnable) 
{
...
void CButtonST_EX::OnKillFocus(CWnd * pNewWnd)
{
	CButton::OnKillFocus(pNewWnd);
	CancelHover();
} // End of OnKillFocus

void CButtonST_EX::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
	CButton::OnActivate(nState, pWndOther, bMinimized);
	if (nState == WA_INACTIVE)	CancelHover();
} // End of OnActivate

void CButtonST_EX::OnCancelMode() 
{
	CButton::OnCancelMode();
	CancelHover();
} // End of OnCancelMode

BOOL CButtonST_EX::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
void CButtonST_EX::OnMouseMove(UINT nFlags, CPoint point)
{
...
LRESULT CButtonST_EX::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
	CancelHover();
...
BOOL CButtonST_EX::OnClicked() 
{
	...
	else
	{
...
#endif
		{
			...
			DWORD dwRetValue = psub->TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON |TPM_NONOTIFY | TPM_RETURNCMD, rWnd.left, rWnd.bottom, this, NULL);
#else
			...
			DWORD dwRetValue = ::TrackPopupMenuEx(hSubMenu, TPM_LEFTALIGN | TPM_LEFTBUTTON |TPM_NONOTIFY | TPM_RETURNCMD, rWnd.left, rWnd.bottom, m_hParentWndMenu, NULL);
#endif
...
void CButtonST_EX::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	//	DrawTheText(pDC, (LPCTSTR)sTitle, &lpDIS->rcItem, &captionRect, m_bIsPressed, m_bIsDisabled);
		CRect rcCaption;
		rcCaption.left = lpDIS->rcItem.left;
		rcCaption.top = lpDIS->rcItem.top;
		rcCaption.right = lpDIS->rcItem.right;
		rcCaption.bottom = lpDIS->rcItem.bottom;
		DrawTheText(pDC, (LPCTSTR)sTitle, &lpDIS->rcItem, &rcCaption, m_bIsPressed, m_bIsDisabled);
	} // if
...
void CButtonST_EX::PaintBk(CDC* pDC)
{
...
HBITMAP CButtonST_EX::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
...
//
void CButtonST_EX::PrepareImageRect(BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, DWORD dwWidth, DWORD dwHeight, CRect* rpImage)
{
	CRect rBtn;
...
void CButtonST_EX::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	BYTE		byIndex		= 0;
	...
	pDC->DrawState(	rImage.TopLeft(),
					...
					(bIsDisabled ? DSS_NORMAL : DSS_NORMAL), 
					(CBrush*)NULL);
} // End of DrawTheIcon

void CButtonST_EX::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	HDC			hdcBmpMem	= NULL;
...
void CButtonST_EX::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
...
//
HBITMAP CButtonST_EX::CreateGrayscaleBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans)
{
...
//		If the function fails, the return value is NULL.
//
HBITMAP CButtonST_EX::CreateDarkerBitmap(HBITMAP hBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTrans)
{
	HBITMAP		hGrayBitmap = NULL;
	HDC			hMainDC = NULL, hMemDC1 = NULL, hMemDC2 = NULL;
	HBITMAP		hOldBmp1 = NULL, hOldBmp2 = NULL;
...
//
HICON CButtonST_EX::CreateGrayscaleIcon(HICON hIcon)
{
...
//		If the function fails, the return value is NULL.
//
HICON CButtonST_EX::CreateDarkerIcon(HICON hIcon)
{
	HICON		hGrayIcon = NULL;
	HDC			hMainDC = NULL, hMemDC1 = NULL, hMemDC2 = NULL;
	BITMAP		bmp;
...
COLORREF CButtonST_EX::DarkenColor(COLORREF crColor, double dFactor)
{
...
//
DWORD CButtonST_EX::SetIcon(int nIconIn, int nCxDesiredIn, int nCyDesiredIn, int nIconOut, int nCxDesiredOut, int nCyDesiredOut)
{
	HICON		hIconIn			= NULL;
	HICON		hIconOut		= NULL;
...
//			Failed loading the specified resource.
//
DWORD CButtonST_EX::SetIcon(int nIconIn, int nIconOut)
{
...
//			Failed loading the specified resource.
//
DWORD CButtonST_EX::SetIcon(HICON hIconIn, HICON hIconOut)
{
	BOOL		bRetValue;
	ICONINFO	ii;
...
//
DWORD CButtonST_EX::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut,int nBitmapDown, COLORREF crTransColorDown)
{
	HBITMAP		hBitmapIn		= NULL;
	HBITMAP		hBitmapOut		= NULL;
	HBITMAP     hBitmapDown		= NULL;
	HINSTANCE	hInstResource	= NULL;
	...
	switch (nBitmapDown)
	{
		case NULL:
			break;
		case (int)BTNST_AUTO_GRAY:
			hBitmapDown = (HBITMAP)BTNST_AUTO_GRAY;
			break;
		case (int)BTNST_AUTO_DARKER:
			hBitmapDown = (HBITMAP)BTNST_AUTO_DARKER;
			break;
		default:
			hBitmapDown = (HBITMAP)::LoadImage(hInstResource, MAKEINTRESOURCE(nBitmapDown), IMAGE_BITMAP, 0, 0, 0);
			if( hBitmapDown)
			{
				m_csBitmaps[2].bDelete = FALSE;
			}
			break;
	} 
	return SetBitmaps(hBitmapIn, crTransColorIn, hBitmapOut, crTransColorOut,hBitmapDown,crTransColorDown);
} // End of SetBitmaps
...
//			Failed creating mask bitmap.
//
DWORD CButtonST_EX::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut,HBITMAP hBitmapDown, COLORREF crTransColorDown)
{
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
	...
	if (hBitmapIn)
	{
		...
		if (hBitmapDown)
		{
			m_csBitmaps[2].hBitmap = hBitmapDown;
			m_csBitmaps[2].crTransparent = crTransColorDown;
			// Get bitmap size
			nRetValue = ::GetObject(hBitmapDown, sizeof(csBitmapSize), &csBitmapSize);
			if (nRetValue == 0)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
			...
			// Create mask for bitmap Out
			m_csBitmaps[2].hMask = CreateBitmapMask(hBitmapDown, m_csBitmaps[2].dwWidth, m_csBitmaps[2].dwHeight, crTransColorDown);
...
//				By default, CButtonST_EX buttons are flat.
//		[IN]	bRepaint
...
//			Function executed successfully.
//
DWORD CButtonST_EX::SetFlat(BOOL bFlat, BOOL bRepaint)
{
	m_bIsFlat = bFlat;
...
//				By default, CButtonST_EX buttons have ST_ALIGN_HORIZ alignment.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CButtonST_EX::SetAlign(BYTE byAlign, BOOL bRepaint)
{
	switch (byAlign)
	{    
		case ST_ALIGN_HORIZ:
...
//				By default, CButtonST_EX buttons have BTNST_PRESSED_LEFTRIGHT style.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//
DWORD CButtonST_EX::SetPressedStyle(BYTE byStyle, BOOL bRepaint)
{
	switch (byStyle)
	{
...
//			Function executed successfully.
//
DWORD CButtonST_EX::SetCheck(int nCheck, BOOL bRepaint)
{
	if (m_bIsCheckBox)
	{
...
//			Function executed successfully.
//
DWORD CButtonST_EX::SetDefaultColors(BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CButtonST_EX::SetColor(BYTE byColorIndex, COLORREF crColor, BOOL bRepaint)
{
...
//			Invalid color index.
//
DWORD CButtonST_EX::GetColor(BYTE byColorIndex, COLORREF* crpColor)
{
	if (byColorIndex >= BTNST_MAX_COLORS)	return BTNST_INVALIDINDEX;
...
//
DWORD CButtonST_EX::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
	BYTE	byRed = 0;
	BYTE	byGreen = 0;
	BYTE	byBlue = 0;
...
//			Function executed successfully.
//
DWORD CButtonST_EX::SetAlwaysTrack(BOOL bAlwaysTrack)
{
...
//			Failed loading the specified resource.
//
DWORD CButtonST_EX::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	HINSTANCE	hInstResource = NULL;
...
//			Function executed successfully.
//
DWORD CButtonST_EX::DrawBorder(BOOL bDrawBorder, BOOL bRepaint)
{
...
//			Function executed successfully.
//
DWORD CButtonST_EX::DrawFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
//
void CButtonST_EX::SetTooltipText(int nText, BOOL bActivate)
{
	CString sText;
...
//				If TRUE the tooltip will be created active.
//
void CButtonST_EX::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
//
void CButtonST_EX::ActivateTooltip(BOOL bActivate)
{
...
//			Function executed successfully.
//
DWORD CButtonST_EX::EnableBalloonTooltip()
{
...
//				If TRUE the control will be repainted.
//
void CButtonST_EX::DrawTransparent(BOOL bRepaint)
{
...
DWORD CButtonST_EX::SetBk(CDC* pDC)
{
...
//			Function executed successfully.
//
DWORD CButtonST_EX::SetURL(LPCTSTR lpszURL)
{
...
//
#ifndef	BTNST_USE_BCMENU
DWORD CButtonST_EX::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint)
{
	HINSTANCE	hInstResource	= NULL;
...
//
#ifdef	BTNST_USE_BCMENU
DWORD CButtonST_EX::SetMenu(UINT nMenu, HWND hParentWnd, BOOL bWinXPStyle, UINT nToolbarID, CSize sizeToolbarIcon, COLORREF crToolbarBk, BOOL bRepaint)
{
	BOOL	bRetValue = FALSE;
...
//			Function executed successfully.
//
DWORD CButtonST_EX::SetMenuCallback(HWND hWnd, UINT nMessage, LPARAM lParam)
{
...
//
void CButtonST_EX::SizeToContent()
{
	if (m_csIcons[0].hIcon)
	{
		m_ptImageOrg.x = 0;
		m_ptImageOrg.y = 0;
	    SetWindowPos(	NULL, -1, -1, m_csIcons[0].dwWidth-2, m_csIcons[0].dwHeight-2,
...
//
#ifdef	BTNST_USE_SOUND
DWORD CButtonST_EX::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
	...
	if (lpszSound)
	{
		...
		else
		{
            CString strTemp;
            strTemp = lpszSound;
			_tcscpy(m_csSounds[byIndex].szSound, strTemp); //这里若将函数参数lpszSound作为_tcscpy第二个参数，是一个高风险的缺陷，coverity报错
...
// This is a virtual function that can be rewritten in CButtonST_EX-derived classes
...
//			Function executed successfully.
//
DWORD CButtonST_EX::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	COLORREF	crColor;
...
//			Function executed successfully.
//
DWORD CButtonST_EX::OnDrawBorder(CDC* pDC, CRect* pRect)
{
	// Draw pressed button
	if (m_bIsPressed)
	{
