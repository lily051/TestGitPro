...
void CBitmapSlider::OnPaint() 
{
	...
	CMemDC_EX dc( &dcOrigin, &m_rect, m_bTransparentChannel );
	...
	if (NULL == dcMem.m_hDC)
	{
		OutputDebugString(_T("CBitmapSlider  dcMem.m_hDC == NULL..............."));
		return;
	}
	...
	if( m_bThumb && m_bEnable ) {
		...
		CRect rcText(0,0,m_nThumbWidth, m_nThumbHeight);
		dcMem.FillSolidRect(rcText, NULL_BRUSH);
	...
	if (m_bThumbText)
	{
		//创建缓冲dc,画布大小与Thumb所在部分大小相同
		CBitmap bmShow;
		bmShow.CreateCompatibleBitmap(&dcOrigin, m_nThumbWidth, m_nThumbHeight);
		CDC dcShow;
		dcShow.CreateCompatibleDC(&dcOrigin);
		if (NULL == dcShow.m_hDC)
		{
			dcMem.DeleteDC();
			return;
		}
		dcShow.SelectObject(&bmShow);
		//从整体的背景dc中将Thumb所在的部分dc copy到缓冲dc中
		dcShow.SetStretchBltMode(HALFTONE);
		dcShow.SetBrushOrg(0, 0);
		dcShow.StretchBlt(0, 0, m_nThumbWidth, m_nThumbHeight, &dc, Pos2Pixel(m_nPos) - m_nThumbWidth/2, m_nMarginTop, m_nThumbWidth, m_nThumbHeight, SRCCOPY);
		//写字
		dcShow.SetBkMode(TRANSPARENT);
		dcShow.SetTextColor(RGB(25, 25, 25));
		dcShow.SelectObject(&m_font);
		UINT format = DT_SINGLELINE | DT_VCENTER | DT_CENTER;
		dcShow.DrawText(m_strWindowText, CRect(0,0,m_nThumbWidth, m_nThumbHeight), format);
		//将写好字的缓冲dc copy到背景dc覆盖Thumb部分
		dc.BitBlt(Pos2Pixel(m_nPos) - m_nThumbWidth/2, m_nMarginTop, m_nThumbWidth, m_nThumbHeight, &dcShow, 0, 0, SRCCOPY);
		//删除缓冲dc
		bmShow.DeleteObject();
		dcShow.DeleteDC();
	}
...
BOOL CBitmapSlider::LoadPngChannel( UINT nChannelID, UINT nActiveID , BOOL bTransparent,
									 COLORREF clrpTransColor, int iTransPixelX, int iTransPixelY )
{
	// This control will not have any bitmap for channel
	if( !nChannelID ) {
	...
	// load a bitmap
	m_bmChannel.DeleteObject();
	HBITMAP hBmChannel = RESOURCELOAD_PNG_EX_HBMPE(nChannelID);
	if (hBmChannel == NULL)
	{
		return FALSE;
	}
	m_bmChannel.Attach(hBmChannel);
	...
	if( nActiveID ) {
		...
		HBITMAP hBmActive = RESOURCELOAD_PNG_EX_HBMPE(nActiveID);
		if (hBmActive == NULL)
		{
		    m_bmChannel.DeleteObject();
			if( bTransparent )
		...
		m_bmChannelActive.Attach(hBmActive);
...
BOOL CBitmapSlider::SetBitmapChannel_EX(int nWidth, int nHeight, UINT nChannelID, 
										UINT nActiveID/* =NULL */, BOOL bTransparent/* =FALSE */, 
										COLORREF clrpTransColor/* =0xFF000000 */, int iTransPixelX/* =0 */, 
										int iTransPixelY/* =0 */)
{
	// This control will not have any bitmap for channel
	if( !nChannelID ) {
	...
	{
		CBitmap tempBitmap;
		tempBitmap.LoadBitmap(nChannelID);
		HBITMAP hBitmap = CopyBitmap(tempBitmap, nWidth, nHeight);
		m_bmChannel.Attach(hBitmap);
	}
	...
	if( nActiveID ) {
		...
		{
			CBitmap tempBitmap;
			tempBitmap.LoadBitmap(nActiveID);
			HBITMAP hBitmap = CopyBitmap(tempBitmap, nWidth, nHeight);
			m_bmChannelActive.Attach(hBitmap);
		}
...
BOOL CBitmapSlider::LoadPngChannel_EX(int nWidth, int nHeight, UINT nChannelID, 
										UINT nActiveID/* =NULL */, BOOL bTransparent/* =FALSE */, 
										COLORREF clrpTransColor/* =0xFF000000 */, int iTransPixelX/* =0 */, 
										int iTransPixelY/* =0 */)
{
	...
	////先将图片放大
	{
		HBITMAP hBitmap = RESOURCELOAD_PNG_EX_HBMPE(nChannelID);
		CBitmap tempBitmap;
		tempBitmap.Attach(hBitmap);
		BITMAP	bitmap;
		tempBitmap.GetBitmap( &bitmap );
		int nCpHeight = nHeight <= 0?bitmap.bmHeight:nHeight;
		hBitmap = CopyBitmap(tempBitmap, nWidth, nCpHeight);
		m_bmChannel.Attach(hBitmap);
	}
	...
	if( nActiveID ) {
		...
		{
			HBITMAP hBitmap = RESOURCELOAD_PNG_EX_HBMPE(nActiveID);
			CBitmap tempBitmap;
			tempBitmap.Attach(hBitmap);
			BITMAP	bitmap;
			tempBitmap.GetBitmap( &bitmap );
...
BOOL CBitmapSlider::LoadPngThumb(
								   UINT nThumbID, UINT nActiveID, BOOL bTransparent,
								   COLORREF clrpTransColor, int iTransPixelX, int iTransPixelY )
{
	// This control will not have bitmap
	...
	HBITMAP hBmThumb = RESOURCELOAD_PNG_EX_HBMPE(nThumbID);
	if (hBmThumb == NULL)
	{
		return FALSE;
	}

	m_bmThumb.Attach(hBmThumb);
	...
	if( nActiveID ) {
		...
		HBITMAP hBmThumb = RESOURCELOAD_PNG_EX_HBMPE(nActiveID);
		if (hBmThumb == NULL)
		{
			m_bmThumb.DeleteObject();
			if( bTransparent )
		...
		m_bmThumbActive.Attach(hBmThumb);
...
BOOL CBitmapSlider::PreTranslateMessage(MSG* pMsg)
{
...
void CBitmapSlider::InitToolTip()
{
...
void CBitmapSlider::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
...
void CBitmapSlider::ActivateTooltip(BOOL bActivate)
{
...
void CBitmapSlider::EnableMove(BOOL bEnable)
{
	m_bEnableMove = bEnable;
}
//返回是否可移动
BOOL CBitmapSlider::IsEnableMove()
{
	return m_bEnableMove;
}

void CBitmapSlider::SetParenWnd(HWND hParentWnd)
{
	m_hParentHwnd = hParentWnd;
}

void CBitmapSlider::DrawUiText(CString strMsg)
{

	if (!m_bThumbText)
	{
		m_font.CreateFont(14, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET, 
			OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH|FF_MODERN, _T("Arial"));

		m_bThumbText = TRUE;
	}

	m_strWindowText = strMsg;
