...
HBITMAP CopyBitmap(HBITMAP  hSourceHbitmap, int iWidth, int iHeight)
{
	...
	HBITMAP   hbmResult   =   ::CreateCompatibleBitmap(CClientDC(NULL),   iWidth,   iHeight);   

	HBITMAP   hbmOldSource   =   (HBITMAP)::SelectObject(   sourceDC.m_hDC,   hSourceHbitmap);   
	HBITMAP   hbmOldDest   =   (HBITMAP)::SelectObject(   destDC.m_hDC,   hbmResult   );   
	destDC.StretchBlt(0,0,iWidth,   iHeight,   &sourceDC,   0,   0, bm.bmWidth,  bm.bmHeight, SRCCOPY   );   
...
//能够设置位图的宽度和高度
CBitItem::CBitItem(UINT nResID, int cx, int cy, int iWidth, int iHeight, int nMode/* = CBitItem::kBitReadMode*/)
	: m_nMode(nMode),
	m_nItemNums(0),
	m_nItemWidth(cx),
	m_nItemHeight(cy),
	...
	m_Offset(0, 0)
{
	...
	//先将图片放大
	CBitmap tempBitmap;
	tempBitmap.LoadBitmap(nResID);
	HBITMAP hBitmap = CopyBitmap(tempBitmap, iWidth, iHeight);
	m_ReadBit.Attach(hBitmap);
	bRet = TRUE;
	//]]end
