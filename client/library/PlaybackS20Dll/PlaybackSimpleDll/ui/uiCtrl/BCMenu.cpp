...
Win32Type IsShellType()
{
	...
	//modify by yudan at2012-6-26 15:13

    if ( dwMajor == 6 && dwMinor == 2)
    {
        ShellType = Win8;
    }
    else if (dwMajor ==6 && dwMinor ==1)  //win7
	{
		ShellType = Win7;
	}
	else if (dwMajor ==6 && dwMinor ==0)  //vista
	{
		ShellType = WinVista;
	}
	else if (dwMajor ==5 && dwMinor ==2)  //2003
	{
		ShellType = WinNT3;
	}
	else if (dwMajor == 5 && dwMinor == 1)  //xp
	{
		ShellType = WinXP;
	}
	else if (dwMajor == 5 && dwMinor == 0)  //2000
	{
		ShellType = Win2000;
	}
	else if (dwMajor == 4 && dwMinor == 1)  //win98
	{
		ShellType = Win98;
	}
	else if (dwMajor == 4 && dwMinor == 0)  //95
	{
		ShellType = Win95;
	}
	else
...
// 			else if(osvi->dwMajorVersion==5L&&osvi->dwMinorVersion==0L)ShellType=WinXP;       //myl20080321:解决win2000下菜单显示问题
...
{
	//modify by yd at 2012-6-26 15:13 add Win7 and WinVista type as xp modle  
	if(g_Shell==WinXP || g_Shell==WinVista || g_Shell== Win7 || g_Shell == Win8){
...
*/			
			if(state&ODS_CHECKED && nIconNormal<0){
				...
				else if(state&ODS_GRAYED) disableflag=TRUE;//gll
			}
		}
		else{
			strText.Empty();
		...
		if(checkflag||standardflag||selectedflag||disableflag){
			...
			if(checkflag && checkmaps){
				...
				if(state&ODS_SELECTED)/*checkmaps*/bitmap->Draw(pDC,1,ptImage,ILD_TRANSPARENT);
				else/* checkmaps*/bitmap->Draw(pDC,0,ptImage,ILD_TRANSPARENT);
			}
			else if(disableflag){
				if(!selectedflag){
...
void BCMenu::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	...
	CRect rect,rect2;
	UINT state = (((BCMenuData*)(lpDIS->itemData))->nFlags);
// 修改菜单风格 [5/22/2009 guolinlin]	COLORREF m_newclrBack=RGB(146,183,255);//gllGetSysColor(COLOR_3DFACE);
	COLORREF m_newclrBack=RGB(112,112,112);	//修改菜单风格  [5/22/2009 guolinlin]
// 修改菜单风格 [5/22/2009 guolinlin]	COLORREF m_clrBack=GetSysColor(COLOR_WINDOW);
	COLORREF m_clrBack=RGB(145,143,144);// 修改菜单风格 [5/22/2009 guolinlin]
	CFont m_fontMenu,*pFont=NULL;
	LOGFONT m_lf;
//修改菜单风格  [5/22/2009 guolinlin]	if(!IsWinXPLuna())m_newclrBack=RGB(146,183,255);//gll//LightenColor(m_newclrBack,0.25);//(30.102.184)
	if(!IsWinXPLuna())m_newclrBack=RGB(112,112,112);// 修改菜单风格 [5/22/2009 guolinlin]//gll//LightenColor(m_newclrBack,0.25);//(30.102.184)
	...
	if(state & MF_SEPARATOR){
		...
		CPen pen;
//  修改菜单风格[5/22/2009 guolinlin]		pen.CreatePen(PS_SOLID,1,RGB(170,194,218));
		pen.CreatePen(PS_SOLID,1,RGB(172,168,153));// 修改菜单风格 [5/22/2009 guolinlin]
//		pDC->DrawEdge(&rect,EDGE_ETCHED,BF_TOP);
		pDC->SelectObject(&pen);
		pDC->MoveTo(rect.left,rect.top);
		pDC->LineTo(rect.right,rect.top);
		//AppFun_FreeImageResource(pen.m_hObject);
		//ljg 2010/03/19 add [[
		if (pen.m_hObject)
		{
		...
		BOOL standardflag=FALSE,selectedflag=FALSE,disableflag=FALSE;
		BOOL checkflag=FALSE;
// 修改菜单风格 [5/22/2009 guolinlin]		COLORREF crText = RGB(0,0,128);//gll//GetSysColor(COLOR_MENUTEXT);
		COLORREF crText = RGB(0,0,0);// 修改菜单风格 [5/22/2009 guolinlin]//gll//GetSysColor(COLOR_MENUTEXT);
		COLORREF crTextSel = RGB(255,255,255);// 修改菜单风格 [5/22/2009 guolinlin]//gll//GetSysColor(COLOR_MENUTEXT);
// 修改菜单风格 [5/22/2009 guolinlin]		COLORREF crSelect = RGB(134,165,196);//gll//GetSysColor(COLOR_HIGHLIGHT);
		COLORREF crSelect = RGB(188,85,85);// 修改菜单风格 [5/22/2009 guolinlin]//gll//GetSysColor(COLOR_HIGHLIGHT);
		COLORREF crSelectFill;
// 修改菜单风格 [5/22/2009 guolinlin]		if(!IsWinXPLuna())crSelectFill=RGB(207,231,255);//gllLightenColor(crSelect,0.85);
		if(!IsWinXPLuna())crSelectFill=RGB(137,41,42);// 修改菜单风格 [5/22/2009 guolinlin]	
// 修改菜单风格[5/22/2009 guolinlin]		else crSelectFill=RGB(207,231,255); //gllLightenColor(crSelect,0.7);
		else crSelectFill=RGB(137,41,42); // 修改菜单风格 [5/22/2009 guolinlin]//gllLightenColor(crSelect,0.7);
		CBrush m_brSelect;
		CPen m_penBack;
		int x0,y0,dx,dy;
		int nIconNormal=-1,xoffset=-1,global_offset=-1;
		...
		if(lpDIS->itemData != NULL){
			...
			xoffset = (((BCMenuData*)(lpDIS->itemData))->xoffset);
			bitmap = (((BCMenuData*)(lpDIS->itemData))->bitmap);
			//TRACE("lpDIS->itemData = %d",lpDIS->itemData);
			strText = ((BCMenuData*) (lpDIS->itemData))->GetString();
			global_offset = (((BCMenuData*)(lpDIS->itemData))->global_offset);
			...
			if(state&ODS_CHECKED && nIconNormal<0){
				...
				standardflag=TRUE;
				if(state&ODS_SELECTED && !(state&ODS_GRAYED))selectedflag=TRUE;
				else if(state&ODS_GRAYED) disableflag=FALSE;//gll
			}
		}
		else{
			strText.Empty();
		...
		if(checkflag||standardflag||selectedflag||disableflag){
			...
			if(checkflag && checkmaps){
				...
				if(state&ODS_SELECTED)bitmap->Draw(pDC,-1,ptImage,ILD_TRANSPARENT);//gll
				/*else / *checkmaps* /bitmap->Draw(pDC,-1,ptImage,ILD_TRANSPARENT);*///gll
			}
			else if(disableflag){
				if(!selectedflag){
					CBitmap bitmapstandard;
...
//						bitmap->Draw(pDC,shadow_offset,ptImage1,ILD_TRANSPARENT);
						bitmap->Draw(pDC,xoffset,ptImage2,ILD_TRANSPARENT);//gll
					}
					else bitmap->Draw(pDC,xoffset,ptImage,ILD_TRANSPARENT);//gll
				}
			}
			else{
				if(state&ODS_CHECKED){
					...
					pDC->FillRect (rect2,&m_newbrBackground);
					pDC->Draw3dRect (rect2,m_newclrBack,m_newclrBack);
					CPoint ptImage(rect.left+dx,rect.top+dy - 1);
					if(bitmap){
						if(CanDraw3D && (state&ODS_GRAYED))//gll0326
						{
							bitmap->Draw(pDC,faded_offset,ptImage,ILD_TRANSPARENT);//gll
							//TRACE("faded_offset = %d",faded_offset);
						}
						else
		...
		if(!strText.IsEmpty()){
			...
			if(!(lpDIS->itemState & ODS_GRAYED)){
				if(state&ODS_SELECTED)
					pDC->SetTextColor(crTextSel);//gll
				else
					pDC->SetTextColor(crText);
				pDC->DrawText (leftStr,rectt,nFormat);
				if(tablocr!=-1) pDC->DrawText (rightStr,rectt,nFormatr);
