...
#include "memdc_EX.h" // "Flicker Free Drawing In MFC" by Keith Rule
...
public:
	...
	//ljg add 2010-4-19 18:48:38 增加将位图放大的大小
	BOOL SetBitmapChannel_EX(int nWidth, int nHeight,
		UINT nChannelID, UINT nActiveID=NULL, BOOL bTransparent=FALSE,
		COLORREF clrpTransColor=0xFF000000, int iTransPixelX=0, int iTransPixelY=0);

	BOOL LoadPngChannel_EX(int nWidth, int nHeight,
		UINT nChannelID, UINT nActiveID=NULL, BOOL bTransparent=FALSE,
		COLORREF clrpTransColor=0xFF000000, int iTransPixelX=0, int iTransPixelY=0);
...
public:
	...
	BOOL m_bEnableMove;      //是否可以移动滑动块

	BOOL IsEnableMove();
	void SetParenWnd(HWND hParentWnd);
	...
	void DrawUiText(CString strMsg);
