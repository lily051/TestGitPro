...
// 修改菜单风格[5/22/2009 guolinlin]		else crSelectFill=RGB(207,231,255); //gllLightenColor(crSelect,0.7);
		...
		if(checkflag||standardflag||selectedflag||disableflag){
			...
			if(checkflag && checkmaps){
				...
				if(state&ODS_SELECTED)bitmap->Draw(pDC,-1,ptImage,ILD_TRANSPARENT);//gll    what the fuck want to do ?
...
BOOL BCMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,COLORREF fill,COLORREF border,int hatchstyle,CSize *pSize)
{
    ...
    pDC = pWnd->GetDC();              // Get device context
    if (NULL == pDC)
    {
        return FALSE;
    }
