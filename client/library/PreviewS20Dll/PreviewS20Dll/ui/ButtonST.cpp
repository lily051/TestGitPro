...
#ifndef	TTM_SETTITLE
#define TTM_SETTITLEA           (WM_USER + 5020)  // wParam = TTI_*, lParam = char* szTitle
#define TTM_SETTITLEW           (TTM_SETTITLEA + 1)  // wParam = TTI_*, lParam = wchar* szTitle
#ifdef	UNICODE
...
IMPLEMENT_DYNAMIC(CButtonST, CButton)//gll090113
...
void CButtonST::FreeResources(BOOL bCheckForNULL)
{
	if (bCheckForNULL)
	{
		...
		if (hBitmapDisabled) ::DeleteObject(hBitmapDisabled);
...
LRESULT CButtonST::OnMouseLeave(WPARAM wParam, LPARAM lParam)
{
    ...
    if (m_ToolTip.GetSafeHwnd() != NULL)
    {
        ...
        if (!m_strTooltipText.IsEmpty())
        {
            ...
            m_ToolTip.AddTool(this, (LPCTSTR)m_strTooltipText, rectBtn, 1);
...
//	if(bIsDisabled)
//	{
//		hbmOldBmp = (HBITMAP)::SelectObject(hdcBmpMem, hBitmapDisabled);
//	}
//	else
	{
	...
	if (hGrayBitmap)
	{
		...
		for (dwLoopY = 0; dwLoopY < dwHeight; dwLoopY++)
		{
			for (dwLoopX = 0; dwLoopX < dwWidth; dwLoopX++)
			{
				...
				{
					::SetPixel(hMemDC1, dwLoopX, dwLoopY, /*crPixel*/RGB(130, 130, 130));
				}
			} // for
		} // for
...
//	FreeResources();
	...
	if (hBitmapIn)
	{
		...
		hBitmapDisabled = CreateGrayscaleBitmap(hBitmapIn, m_csBitmaps[0].dwWidth, m_csBitmaps[0].dwHeight, crTransColorIn);
...
void CButtonST::SetTooltipText(int nText, BOOL bActivate)
{
	...
	m_strTooltipText.LoadString(nText);
	// If string resource is not empty
	if (m_strTooltipText.IsEmpty() == FALSE) SetTooltipText((LPCTSTR)m_strTooltipText, bActivate);
...
void CButtonST::UpdateTooltip()
{
    ...
    SetTooltipText((LPCTSTR)m_strTooltipText);
}
...
DWORD CButtonST::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
	...
	if (lpszSound)
	{
		...
		{
            CString strSzSound = lpszSound;
            if (strSzSound.GetLength() <= _MAX_PATH)
            {
                _tcscpy(m_csSounds[byIndex].szSound, strSzSound);
            }
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	...
	if (m_bIsFlat == FALSE)
	{
		if (m_bIsFocused || m_bIsDefault)
		{
			...
			br.DeleteObject();// 删除 [7/16/2009 guolinlin]
		} // if
	} // if
	...
	brBackground.DeleteObject();// 删除画刷 [7/16/2009 guolinlin]
...
DWORD CButtonST::OnDrawBorder(CDC* pDC, CRect* pRect)
{
	...
	if (m_bIsPressed)
	{
		...
		else    
		{
			...
			brBtnShadow.DeleteObject();// 删除画刷 [7/16/2009 guolinlin]
		}
	}
...
#ifdef	BTNST_USE_BCMENU
			BCMenu *psub = (BCMenu*)m_menuPopup.GetSubMenu(0);
			DWORD dwRetValue = psub->TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD, rWnd.left + point.x , rWnd.top + point.y, this, NULL);
#else
			HMENU hSubMenu = ::GetSubMenu(m_hMenu, 0);
			//DWORD dwRetValue = ::TrackPopupMenuEx(hSubMenu, TPM_LEFTALIGN | TPM_LEFTBUTTON, rWnd.left, rWnd.bottom, m_hParentWndMenu, NULL);
			DWORD dwRetValue = ::TrackPopupMenuEx(hSubMenu, TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD,  rWnd.left + point.x , rWnd.top + point.y, m_hParentWndMenu, NULL);
#endif
...
//void CButtonST::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
//{
//	// TODO: Add your message handler code here and/or call default
//
