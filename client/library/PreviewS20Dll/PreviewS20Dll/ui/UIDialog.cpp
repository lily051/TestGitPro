...
#include "..\util\AutoExchangeModuleState.h"
...
{
	m_hIcon = AfxGetApp()->LoadIcon(IDI_ICON_CONTROLCLIENT);
	m_bCenter = FALSE;

    m_bitmap.Attach(m_pngMetrics.LoadPNG(IDB_PNG_MAINDLG, UI_COLOR_TRANSPARENT));
...
BOOL UIDialog::OnInitDialog()
{
	...
	m_brushBkgnd.CreateSolidBrush(theApp.GetUIColor(UI_COLOR_TYPE_UIDIALOG));

    LOGFONT lf = {0};
    lf.lfHeight = UI_FONT_BAR_HEIGHT;
    lf.lfWeight = UI_FONT_BAR_WEIGHT;
    ::lstrcpy(lf.lfFaceName, UI_FONT_BAR_NAME);
...
void UIDialog::DrawDialog(void)
{
	...
	if (NULL != dc.GetSafeHdc())
	{
		...
		memDC.TransparentBlt(rcRight.left,rcRight.top,rcRight.Width(),rcRight.Height(),
			&m_dcBuffer,xSrc,ySrc,wSrc,hSrc,UI_COLOR_TRANSPARENT);
		...
		memDC.SetTextColor(theApp.GetUIColor(UI_COLOR_TYPE_UIDIALOG_TITLE));
...
/*
    ...
    if (NULL == m_hResource)
    {
        m_hResource = LoadLibrary(sResFileName);
    }
    if (m_hResource == NULL)
    {
...
BOOL UIDialog::SystemButton::DrawButton(CDC * pDC, CDC * pBufferDC, BOOL bMax)
{
    ...
    BOOL bOK = pDC->TransparentBlt(m_rc.left,m_rc.top,m_rc.Width(),m_rc.Height(),
        pBufferDC, xSrc + nStart * (m_nWidth + 2),ySrc,m_rc.Width(),m_rc.Height(),UI_COLOR_TRANSPARENT);
