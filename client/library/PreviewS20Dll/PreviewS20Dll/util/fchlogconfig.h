#ifndef GUARD_FCHLOGCONFIG_H
#define GUARD_FCHLOGCONFIG_H
#pragma once
/**	@file    fchlogConfig.h
...
// for Video Service module
#if (defined _WIN32 || defined _WIN64)
#define VSLOG_OBJECT_NAME             "PREVIEW_PLUG"
#define PV_ERROR(fmt, ...)         hlog_format(HLOG_LEVEL_ERROR, VSLOG_OBJECT_NAME, "<%s [%d] - %s> <"fmt">",  __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)
#define PV_WARN(fmt, ...)          hlog_format(HLOG_LEVEL_WARN, VSLOG_OBJECT_NAME, "<%s [%d] - %s> <"fmt">",  __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)    
#define PV_INFO(fmt, ...)          hlog_format(HLOG_LEVEL_INFO, VSLOG_OBJECT_NAME, "<%s [%d] - %s> <"fmt">",  __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)    
#define PV_DEBUG(fmt, ...)         hlog_format(HLOG_LEVEL_DEBUG, VSLOG_OBJECT_NAME, "<%s [%d] - %s> <"fmt">",  __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)    
#define PV_TRACE(fmt, ...)         hlog_format(HLOG_LEVEL_TRACE, VSLOG_OBJECT_NAME, "<%s [%d] - %s> <"fmt">",  __FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)    
#else
...
#endif // end of GUARD_FCHLOGCONFIG_H
