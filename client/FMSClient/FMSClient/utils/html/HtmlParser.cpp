...
void HtmlParser::SetHtmlNodeText(int index, TCHAR* strData)
{
	assert(index >= 0 && index <= getHtmlNodeCount());
	HtmlNode* node = (HtmlNode*)m_HtmlNodes.getData() + index;

	//TCHAR *str= TEXT("id=\"imgStart\" onclick=\"fileOpenDialogStart()\" onmouseover=\"mouseOverStart()\" src=\"D:\\1.jpg\"");
//	_tcscpy_s(str, strlen())
	char* strValue; 
	int textlen; 
	textlen = WideCharToMultiByte( CP_UTF8, 0, strData, -1, NULL, 0, NULL, NULL ); 
	strValue =(char *)malloc((textlen+1)*sizeof(char)); 
	memset(strValue, 0, sizeof(char) * ( textlen + 1 ) ); 
	WideCharToMultiByte( CP_UTF8, 0, strData, -1, strValue, textlen, NULL, NULL ); 
	//return result; 

	setNodeText(node, strValue, strlen(strValue));

}

void HtmlParser::SetAttributeValue(int index, TCHAR * strData)
{
	assert(index >= 0 && index <= getHtmlNodeCount());
	HtmlNode* node = (HtmlNode*)m_HtmlNodes.getData() + index;

	//TCHAR *str= TEXT("D:\\1.jpg");
	char * strAttributeName = "src";
	char* strValue; 
	int textlen; 
	textlen = WideCharToMultiByte( CP_UTF8, 0, strData, -1, NULL, 0, NULL, NULL ); 
	strValue =(char *)malloc((textlen+1)*sizeof(char)); 
	...
	SetAttributeStringValue(node, strAttributeName, strValue);
}

void HtmlParser::SetHtmlNodeAttribute(int index, char* data)
{
// 	assert(index >= 0 && index <= getHtmlNodeCount());
// 	HtmlNode* node = (HtmlNode*)m_HtmlNodes.getData() + index;
// 
...
// 	pResult =(char *)malloc((textlen+1)*sizeof(char)); 
// 	memset(pResult, 0, sizeof(char) * ( textlen + 1 ) ); 
// 	WideCharToMultiByte( CP_UTF8, 0, str, -1, pResult, textlen, NULL, NULL ); 
// 
// 	//setNodeText(node, result, strlen(result));
// 	setNodeAttributeText(node, pResult);
// 	if (NULL != pResult)
// 	{
// 		delete pResult;
// 		pResult = NULL;
...
HtmlAttribute* HtmlParser::getAttribute(const HtmlNode* pNode, size_t index)
{
	return ((HtmlAttribute*) pNode->attributes->getData()) + index;
}

HtmlAttribute* HtmlParser::getAttribute(const HtmlNode* pNode, const char* szAttributeName)
{
	...
	{
		HtmlAttribute* attribute = getAttribute(pNode, i);
...
char* HtmlParser::getAttributeStringValue(const HtmlNode* pNode, const char* szAttributeName, char* szDefaultValue /*= NULL*/)
{
	HtmlAttribute* pAttribute = getAttribute(pNode, szAttributeName);
	if(pAttribute)
...
void HtmlParser::SetAttributeStringValue(const HtmlNode* pNode, char* szAttributeName, char* szAttributeValue)
{
	if(pNode == NULL || pNode->attributeCount <= 0)
		return;
	...
	for (int i = 0; i < pNode->attributeCount; i++)
	{
		...
		if (stricmp(attribute->name, szAttributeName) == 0)
		{
			attribute->value = szAttributeValue;
			return;
		}
	}
}
