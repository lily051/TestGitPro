...
enum HtmlNodeType
{
	...
	_NODE_USER_,    //用户定义的其他节点类型值应大于_NODE_USER_，以确保不与上面定义的常量值重复
};
...
public:
	...
	void SetHtmlNodeText(int index, TCHAR * data);
	void SetAttributeValue(int index, TCHAR * data);
	void SetHtmlNodeAttribute(int index, char *data);
	...
	//attributes
	static HtmlAttribute* getAttribute(const HtmlNode* pNode, size_t index); //must: 0 <= index < pNode->attributeCount
	static HtmlAttribute* getAttribute(const HtmlNode* pNode, const char* szAttributeName); //return NULL if attribute not exist
	static char* getAttributeStringValue(const HtmlNode* pNode, const char* szAttributeName, char* szDefaultValue = NULL);
	static int getAttributeIntValue(const HtmlNode* pNode, const char* szAttributeName, int defaultValue = 0);
	void SetAttributeStringValue(const HtmlNode* pNode, char* szAttributeName, char* szAttributeValue);
