...
BOOL CUpdaterApp::InitInstance()
{
	...
	UPDATER_LOG_INFO("Create local LocalFiles.ini");
	MakeIni *maleIni = new MakeIni;
	maleIni->FindFile(file_info::GetAppDirectory());
	maleIni->WriteIni();
...
BOOL CUpdaterApp::ParseCommandLine()
{
	...
	for (int i = 1; i < __argc; i++)
	{
		...
		if (sParameter.Left(wcslen(PARA_KEY_APP_NAME)).CompareNoCase(PARA_KEY_APP_NAME) == 0)
		{
			m_sAppName = sParameter.Mid(wcslen(PARA_KEY_APP_NAME));
		}
	...
	if (CheckUpdate())
	{
		if (!m_bSilenceMode)
		{
			...
			if (IDOK == MessageBox(NULL, strText, strCaption, MB_ICONQUESTION|MB_OKCANCEL))
			{
				// 进入升级，退出主程序，保证安全顺利升级
				OnIconnotifyStop();
				BOOL bIsExist = IsHaveProgram(m_sAppName+_T(".exe"));
				while(bIsExist == TRUE)
				{
					bIsExist = IsHaveProgram(m_sAppName+_T(".exe"));
				}
		...
		{
			UPDATER_LOG_INFO("Start to Update，exit main program");
			// 进入升级，退出主程序，保证安全顺利升级
			OnIconnotifyStop();
			char temp[MAX_PATH] = {0};
			CloseProgram(m_sAppName + _T(".exe"));
			StringChange::CStringToChar(m_sAppName + _T(".exe"),temp);
			UPDATER_LOG_INFO("Close progress %s ",temp);
			//int error = GetLastError();
			//UPDATER_LOG_INFO("error %d ",error);
			//关闭是否成功
			//BOOL bIsExist = IsHaveProgram(m_sAppName+_T(".exe"));
			//while(bIsExist == TRUE)
			//{
			...
			//	bIsExist = IsHaveProgram(m_sAppName+_T(".exe"));
			//}
			while(IsHaveProgram(m_sAppName+_T(".exe")))
			{
				//int error = GetLastError();
				//UPDATER_LOG_INFO("error %d ",error);
				Sleep(1000);
			}
			UPDATER_LOG_INFO("Close main progress is successful");
		}
	}
	else
	{
...
BOOL CUpdaterApp::CheckUpdate()
{
	...
	//读取本地配置文件
	UPDATER_LOG_INFO("Read LocalFiles.ini");
	ReadLocalInI();
	//读取下载的配置文件
	...
	//获取要更新的文件个数
	int number = 0;
	UpdateFilesNumber(number);
	UPDATER_LOG_INFO("The Number needed to Update is %d",number);
	if ( number > 0)
	{
		return TRUE;
	}
