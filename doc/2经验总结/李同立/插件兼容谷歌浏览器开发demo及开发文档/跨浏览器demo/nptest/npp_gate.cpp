...
{
    return "application/hik-demo-plugin";//to do change type to individual string, which is the type in the html
}
...
NPError NPP_New(NPMIMEType pluginType,
                NPP instance,
                uint16_t mode,
                int16_t argc,            //the number of HTML arguments in the element
                char* argn[],            //The argn array contains the attribute names  
                char* argv[],            //the argv array contains the attribute values
                NPSavedData* saved)
{   
    if(instance == NULL)
    {
...
NPError NPP_SetWindow (NPP instance, NPWindow* pNPWindow)
{
    ...
    if(!pPlugin->isInitialized() && (pNPWindow->window != NULL))
    {
        if(!pPlugin->init(pNPWindow))
        {
            delete pPlugin;
            pPlugin = NULL;
            return NPERR_MODULE_LOAD_FAILED_ERROR;//lint !e438
        }
    }
